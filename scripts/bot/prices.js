...
const { TroveManagerABI } = require('./abi/troveManagerABI.js');
const { CHAINLINK_ABI } = require('./abi/chainlinkABI.js');
const { arbitrageContractABI } = require('./abi/arbitrageABI');
const { ChainId, Token, Fetcher, Route, Trade, TradeType, TokenAmount } = require('@uniswap/sdk');
const { EthersARTH } = require('@mahadao/lib-ethers');
...
const ARTH = new Token(ETH_NETWORK, ADDRESSES[NETWORK]['ARTH'], 6);
...
const arbitrageStatus = async (provider, web3, wallet) => {
	...
	const pair = await Fetcher.fetchPairData(ARTH, WETH, provider);
...
const getFeasibleTrade = async (liquity, pair, wallet) => {
	...
	const arthObtainedFromUni = parseFloat(fromWei(trade.outputAmount.toSignificant(6), 'micro'));

	const populatedRedemption = await liquity.populate
		.redeemARTH(arthObtainedFromUni.toString())
	...
	let redeemableArthAmount = toWei(populatedRedemption.redeemableARTHAmount.toString(), 'ether');
	// Create new trade based on redeemable ARTH amount. This is because the ARTH amount can change.
	trade = new Trade(route, new TokenAmount(ARTH, redeemableArthAmount), TradeType.EXACT_OUTPUT);
...
const constructTx = async (amountIn, decodedRedemptionInput, profit, web3) => {
	const liquityArbitrageContract = new web3.eth.Contract(
		...
		ADDRESSES[NETWORK]['ARTH_ARBITRAGE']
	);
	const tx = liquityArbitrageContract.methods.ethToArthAndBackArbitrage(
