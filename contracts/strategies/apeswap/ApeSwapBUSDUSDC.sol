...
contract ApeSwapBUSDUSDC is IFlashBorrower, ILeverageStrategy {
    ...
    function openPosition(
        ...
        if (finalExposure[0] <= principalCollateral[0]) {
            // We are taking leverage such that: `1 < leverage <= 1.9`.
            (
                uint256 busdToSellForUsdc,
                uint256 flashloanAmountSuggested,
                uint256[2] memory newPrinicpalSuggested
            ) = estimateSwap(principalCollateral[0], finalExposure[0], finalExposure[1]); // Estimate how much we should flashloan based on how much we want to borrow.
            ...
            flashloanAmount = ellipsis // Estimate how much we should flashloan based on how much we want to borrow.
                .estimateARTHtoSell(
                    finalExposure[0].sub(newPrinicpalCollateral[0]),
                    finalExposure[1],
                    0
                )
                .mul(102)
        ...
        )
    {
        ...
        arthToFlashloan = expectedUsdcOut >= usdcOut
            ? 0
            : ellipsis.estimateARTHtoSell(0, usdcOut.sub(expectedUsdcOut), 0).mul(105).div(100);

        newPrincipalCollateral = [busdIn.sub(busdToSellForUsdc), expectedUsdcOut];
    }
